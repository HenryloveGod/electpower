...
#define STOFF	"/tmp/stiff"
#define STIFF "/tmp/stoff"
		...
		if ( FD_ISSET(stiff, &rdset) ) {
			...
			if (n == 0) {
				...
				if ( errno != EAGAIN && errno != EWOULDBLOCK ) 
					...
					goto skip_proc_STIFF;
			}	

			if (fftty_q.len + M_MAXMAP <= TTY_Q_SZ) {
				
				fftty_q.buff[fftty_q.len]=c;
				tty_q.len += 1;
				if ( opts.lecho ) 	map_and_write(STO, opts.emap, c);
			}

		}
skip_proc_STIFF:

		if ( FD_ISSET(stoff, &wrset) ) {
			/* write to port */
			do {
				n = write(stoff, fftty_q.buff, fftty_q.len);
