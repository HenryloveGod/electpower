...
thread_spam(void *d)
{
	...
	do {
		...
		char new_msg[1024]="0";
		int n=0;
		if (n = read(stoff, new_msg, 512) > 0)
		{

			// n = lws_snprintf((char *)amsg.payload + LWS_PRE, len,
			// 				 "%s: tid: %p, msg: %d %s", vhd->config,
			// 				 (void *)pthread_self(), index++, new_msg);

			n = lws_snprintf((char *)amsg.payload + LWS_PRE, len,"%s", new_msg);
			amsg.len = n;
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_RECEIVE:
		...
		recv[len]='\x0d';
		recv[len+1]='\0';
		lwsl_debug("recieve data [%s]\n",recv);

		if((wn = write(stiff,recv,len+2)) ==len +1){
			lwsl_debug("send OK\n",recv);
		}
...
init_fail:
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		...
		char *mm = (char *)(pmsg->payload + LWS_PRE);
		lwsl_debug("%d %s",LWS_PRE,mm);
